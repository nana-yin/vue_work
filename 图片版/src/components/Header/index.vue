      ...
      switch (name) {
        ...
        case '智慧用能':
          ...
          if (hasPermission) { // 有权限，进行跳转
            let sid = ''
            defineData.leftNavPermission.forEach((item:any) => {
              if (item.title === '智慧用能') {
                sid = item.sid
              }
            })
          ...
          request.getStoreSendLoginCode({
            ...
            if (success) {
              ...
              if (model) {
                const { code } = model
                // 路由上面有参数
                url += `/index.html#/index0?code=${code}&loginId=${paramObj.loginId}`
              }
              // console.log('应用中心-拿到的数据', url)
              window.open(url, '_blank')
            }
          })
          break
          ...
          request.getStoreSendLoginCode({
            ...
            if (success) {
              ...
              if (model) {
                const { code } = model
                // 路由上面有参数
                url += `/index.html#/help/index?code=${code}&loginId=${paramObj.loginId}`
              }
              // console.log('知识库-拿到的数据', url)
              window.open(url, '_blank')
            }
          })
          break
