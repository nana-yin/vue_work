...
interface urlType {
  ...
  storeGetUserStoreIds: string;
  storeListModels: string;
}
...
const API: urlType = {
  ...
  storeGetUserStoreIds: EnergyConsumption + '/store/getUserStoreIds', // 获取用户拥有的企业
  storeListModels: EnergyConsumption + '/store/listModels' // 获取所有应用系统下的模块
}
...
export default {
  ...
  // 发送大屏验证码
  getStoreSendLoginCode (params: {
    modelId: string | number;
    loginId: string | number;
    needIndexPage: boolean;
  }) {
    return _$http.get(API.storeSendLoginCode, params)
  },
  ...
  getStoreGetUserStoreIds (params: {
    // 获取用户拥有的企业
    loginId: string;
  }) {
    return _$http.get(API.storeGetUserStoreIds, params)
  },
  // 获取所有应用系统下的模块
  getStoreListModels () {
    return _$http.get(API.storeListModels, {})
  }
}
